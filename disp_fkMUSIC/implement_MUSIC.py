...
import signal_proc.interpolate_pairwise as ip
...
def phasevel_spectrum(fks):
	...
	for j in range(fks.shape[0]):
		...
		try:
			pvs[:,j]=ip.lagrange_linear(cthisf[::-1],fks_thisf[::-1],cfixed)
...
try:
	...
	ftype=None
nmodes=int(input("Number of signals (modes) in the data: "))
if nmodes%2==0:
	efs=1
# efs stands for extra_freq_samples
else:
	efs=2
frange = input("Enter frequency range of interest (Hz): ")
...
cstep=float(input("Phase velocity step: "))
...
if '.SAC' in fid or '.sac' in fid:
	...
	usrc=input('Plot theoretical dispersion too ? (y/n): ')
	if usrc=='y' or usrc=='Y':
		fcspec = plot_spectrum(fc_spectrum,2,True)
		plt.show()
		#plt.savefig('music_fc.png')
usrc2=input("Do you want to save the results ? (y/n) ")
if usrc2=='y':
	store_result()
