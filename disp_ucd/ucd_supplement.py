...
def get_peaks(inmat,gvmin,gvmax,dist,ts,thresh,npick,verbose):
    ...
    while r<=be:
        ...
        while c<=re:
            ...
            win=inmat[r-te:r+te+1,c-int((wwidth-1)/2):c+int((wwidth-1)/2)+1]
...
def make_pickle(wdir,fsamples,cdisp,udisp):
	...
	#pickledir=input("Name of pickle directory: ")
	pickledir='.'
	storethis=list(zip(fsamples,cdisp))
	...
	jarc=open(jarfile,'wb')
	pickle.dump(storethis,jarc)
	jarc.close()
	storethis=list(zip(fsamples,udisp))
	jarname=evdir+"_u"+".pckl"
	jarfile=os.path.join(wdir,pickledir,jarname)
	jaru=open(jarfile,'wb')
...
class workon_pickle():
    ...
    def view_pickle(self):
        ...
        ncols=1
        r=int(self.totp/ncols) if self.totp%ncols==0 else math.ceil(self.totp/ncols)
    ...
    def pick_modes(self):
        ...
        picked_mn=[]
        while usrc_pick=='y':
            pmn=int(input("Mode number to pick: "))
            mfr=input("Frequency range for this pick: ")
            ...
            usrc_pick=input("Pick another mode ? (y/n): ")
        ...
        #	rf=input("Frequency range for pick %d: " %(j+1))
        #	f1=float(rf.split()[0])
        #	f2=float(rf.split()[1])
...
if __name__=='__main__':
    ...
    # path to the "SW1D_earthsr" and "array_proc" set of modules
    ...
    for fn in filnums:
    	fname=sys.argv[fn]
    	flist.append(fname)
    thdpfile=input('File containing theoretical dispersion (just hit ENTER if N/A): ')
    if len(thdpfile)>0:
    ...
    while task != 4:
    	usage()
    	task = int(input("Enter your choice (number) here: "))
    	if task>=4:
    		sys.exit('Thank you')
