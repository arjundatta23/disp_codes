...
def plot_spectrum(showimage,scaleshow,showth,pp):
    ...
    if showth:
        dfile=input('File containing theoretical dispersion: ')
...
#usrc2=input("Normalize v-f spectrum ? (y/n): ")
usrc2='y'
...
if usrc2=='y' or usrc2=='Y':
	...
	if '.SAC' in fid or '.sac' in fid:
		while usrc != 'n':
			usrc=input('Plot theoretical dispersion ? (y/n): ')
			if usrc=='y' or usrc=='Y':
				shot=True
				plot_spectrum(specnorm,False,shot,0)
	while usrc3 != 'n':
		usrc3=input("Plot a pickled file on top (y/n): ")
...
usrc_store=input("Do you want to save the results ? (y/n) ")
if usrc_store=='y':
	store_result()
